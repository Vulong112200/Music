		...
		public function __construct(){
			...
			switch ($act) {
				case 'delete':
					...
					$this->Model->execute("delete from mv where maMV='$id'");
					echo "<meta http-equiv='refresh' content='0; URL=index.php?controller=mv/list'>";
					break;
			}

			// dem tong so ban ghi
			$number = $this->Model->count("select * from mv");
			...
			$data = $this->Model->fetch("select * from mv order by maMV asc limit $form,$num_page");

			include "views/mv/listView.php";
		}
	}
	new ListMV();
